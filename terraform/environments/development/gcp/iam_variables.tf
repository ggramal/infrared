  ...
  iam = {
    ...
    service_accounts = {
      ...
      external-dns = {
        ...
        sa_iam_bindings = {
          "roles/iam.workloadIdentityUser" = [
            "serviceAccount:${var.gcp_projects.dev.name}.svc.id.goog[external-dns/external-dns]",
          ]
        }
        generate_key = false
      },
      ...
      thanos = {
        ...
        sa_iam_bindings = {
          "roles/iam.workloadIdentityUser" = [
            "serviceAccount:${var.gcp_projects.dev.name}.svc.id.goog[prometheus/prometheus-gcp-dev-prometheus-main]",
            "serviceAccount:${var.gcp_projects.dev.name}.svc.id.goog[prometheus/thanos-gcp-dev-dev-compactor]",
          ]
        }
        generate_key = false
      }
      postgres = {
        ...
        sa_iam_bindings = {
          "roles/iam.workloadIdentityUser" = [
            "serviceAccount:${var.gcp_projects.dev.name}.svc.id.goog[postgres/postgres]",
          ]
        }
        generate_key = true
      }
