locals {
  gke_clusters = {
    "int" = {
      ...
      subnetwork              = module.gcp.subnets["main"]["${var.gcp_projects.int.region}/vms"].name
      network                 = module.gcp.vpcs["main"].network_name
      network_policy          = true
      network_policy_provider = "CALICO"
      ip_range_pods           = module.gcp.subnets["main"]["${var.gcp_projects.int.region}/vms"].secondary_ip_range[0].range_name
      ip_range_services       = module.gcp.subnets["main"]["${var.gcp_projects.int.region}/vms"].secondary_ip_range[1].range_name
      ...
      node_pools = [
        {
          ...
          auto_repair        = true
          auto_upgrade       = true
          service_account    = "k8s-nodes@${var.gcp_projects.int.name}.iam.gserviceaccount.com"
          preemptible        = false
          spot               = true
